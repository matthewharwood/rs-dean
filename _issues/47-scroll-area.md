...
Ship a Rust-first `Scroll Area` component that follows the shadcn composition model while using `rs-dean-ui` design tokens, shared component specs, and local-first state boundaries. A custom scroll viewport with tokenized scrollbars and fade affordances.

## Design Considerations

- Use only the shared Tailwind token vocabulary for color, type, space, radius, shadow, and motion.
- Preserve shadcn's compositional anatomy: ScrollArea, ScrollViewport, ScrollContent, ScrollBar, ScrollCorner.
...
- `Scroll Area` appears in the story gallery and `_issues` index.
...
- [x] Concrete component pass complete: typed scroll items, overflow axes, renderer-local viewport/bar state, token-only Leptos viewport, story variants, and Bevy primitives now share the same Rust render nodes.
