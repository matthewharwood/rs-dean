...
Ship a Rust-first `Carousel` component that follows the shadcn composition model while using `rs-dean-ui` design tokens, shared component specs, and local-first state boundaries. A paged horizontal media or content strip with previous/next controls.

## Design Considerations

- Use only the shared Tailwind token vocabulary for color, type, space, radius, shadow, and motion.
- Preserve shadcn's compositional anatomy: Carousel, CarouselContent, CarouselItem, CarouselPrevious, CarouselNext, CarouselIndicator.
...
- `Carousel` appears in the story gallery and `_issues` index.
...
- [x] Concrete typed slide model, density/loop contracts, repeatable item render nodes, validation, renderer-local current-index state, and named token-only Leptos component implemented.
