...
Ship a Rust-first `Alert` component that follows the shadcn composition model while using `rs-dean-ui` design tokens, shared component specs, and local-first state boundaries. A callout that highlights status, risk, or next action.

## Design Considerations

- Use only the shared Tailwind token vocabulary for color, type, space, radius, shadow, and motion.
- Preserve shadcn's compositional anatomy: Alert, AlertTitle, AlertDescription, AlertAction.
...
- `Alert` appears in the story gallery and `_issues` index.
