...
Ship a Rust-first `Input` component that follows the shadcn composition model while using `rs-dean-ui` design tokens, shared component specs, and local-first state boundaries. A single-line text entry control.

## Design Considerations

- Use only the shared Tailwind token vocabulary for color, type, space, radius, shadow, and motion.
- Preserve shadcn's compositional anatomy: Input, InputPrefix, InputControl, InputSuffix.
...
- `Input` appears in the story gallery and `_issues` index.
...
- [x] Concrete prefix/control/suffix model, input kind, optional error state, renderer-local draft state, token-only renderer, story proof, and Bevy primitive derivation implemented.
