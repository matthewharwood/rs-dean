...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/layout/list-containers#component-a1c9099f000c8917f7c633cbfc912084

Catalog path: Application UI / layout / List containers / variant 2 of 7

## TPM Outcome

Ship `application-ui-layout-list-containers-02-card-with-dividers` as a registered Rust-first block in the List containers family. Preserve
...
- Block index `515` and `application-ui-layout-list-containers-02-card-with-dividers` are the stable registry identity.
...
- Resolve `BlockId::from_index(515)`, or look up `application-ui-layout-list-containers-02-card-with-dividers` with
...
- The registry resolves both block id `515` and slug `application-ui-layout-list-containers-02-card-with-dividers` one-to-one.
...
- An isolated `block-application-ui-layout-list-containers-02-card-with-dividers` story proves the fixture in both renderers.
