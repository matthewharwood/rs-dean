...
Source: https://tailwindcss.com/plus/ui-blocks/ecommerce/components/store-navigation#component-161ab7578bf4876db8a1bc43f9b1be8f

Catalog path: Ecommerce / components / Store Navigation / variant 1 of 5

## TPM Outcome

Ship `ecommerce-components-store-navigation-01-with-image-grid` as a registered Rust-first block in the Store Navigation family. Preserve
...
- Block index `589` and `ecommerce-components-store-navigation-01-with-image-grid` are the stable registry identity.
...
- Variant behavior is constrained by `Image`, `Plain`, and `Menu`
...
- Resolve `BlockId::from_index(589)`, or look up `ecommerce-components-store-navigation-01-with-image-grid` with
...
- The registry resolves both block id `589` and slug `ecommerce-components-store-navigation-01-with-image-grid` one-to-one.
...
- An isolated `block-ecommerce-components-store-navigation-01-with-image-grid` story proves the fixture in both renderers.
