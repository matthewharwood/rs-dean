...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/lists/tables#component-38a65f3cd00435cddd893ece030d365e

Catalog path: Application UI / lists / Tables / variant 4 of 19

## TPM Outcome

Ship `application-ui-lists-tables-04-full-width-with-constrained-content` as a registered Rust-first block in the Tables family. Preserve
...
- Block index `264` and `application-ui-lists-tables-04-full-width-with-constrained-content` are the stable registry identity.
...
- The family maps to the `Data display` composition pattern and the `Table`
...
- Resolve `BlockId::from_index(264)`, or look up `application-ui-lists-tables-04-full-width-with-constrained-content` with
...
- The registry resolves both block id `264` and slug `application-ui-lists-tables-04-full-width-with-constrained-content` one-to-one.
...
- An isolated `block-application-ui-lists-tables-04-full-width-with-constrained-content` story proves the fixture in both renderers.
