...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/elements/buttons#component-612a41e698bb076353c4922676292d13

Catalog path: Application UI / elements / Buttons / variant 5 of 8

## TPM Outcome

Ship `application-ui-elements-buttons-05-buttons-with-trailing-icon` as a registered Rust-first block in the Buttons family. Preserve
...
- Block index `490` and `application-ui-elements-buttons-05-buttons-with-trailing-icon` are the stable registry identity.
...
- The family maps to the `Component composition` composition pattern and the `Button`
...
- Resolve `BlockId::from_index(490)`, or look up `application-ui-elements-buttons-05-buttons-with-trailing-icon` with
...
- The registry resolves both block id `490` and slug `application-ui-elements-buttons-05-buttons-with-trailing-icon` one-to-one.
...
- An isolated `block-application-ui-elements-buttons-05-buttons-with-trailing-icon` story proves the fixture in both renderers.
