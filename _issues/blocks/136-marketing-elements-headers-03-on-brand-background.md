...
Source: https://tailwindcss.com/plus/ui-blocks/marketing/elements/headers#component-048b06b7f859d2fa194752ee8805992a

Catalog path: Marketing / elements / Headers / variant 3 of 11

## TPM Outcome

Ship `marketing-elements-headers-03-on-brand-background` as a registered Rust-first block in the Headers family. Preserve
...
- Block index `135` and `marketing-elements-headers-03-on-brand-background` are the stable registry identity.
...
- Variant behavior is constrained by `No media`, `Brand`, and `Menu`
...
- Resolve `BlockId::from_index(135)`, or look up `marketing-elements-headers-03-on-brand-background` with
...
- The registry resolves both block id `135` and slug `marketing-elements-headers-03-on-brand-background` one-to-one.
...
- An isolated `block-marketing-elements-headers-03-on-brand-background` story proves the fixture in both renderers.
