...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/feedback/alerts#component-8a099d76389c1c4d6d4e92d69acbc8d2

Catalog path: Application UI / feedback / Alerts / variant 1 of 6

## TPM Outcome

Ship `application-ui-feedback-alerts-01-with-description` as a registered Rust-first block in the Alerts family. Preserve
...
- Block index `364` and `application-ui-feedback-alerts-01-with-description` are the stable registry identity.
...
- Resolve `BlockId::from_index(364)`, or look up `application-ui-feedback-alerts-01-with-description` with
...
- The registry resolves both block id `364` and slug `application-ui-feedback-alerts-01-with-description` one-to-one.
...
- An isolated `block-application-ui-feedback-alerts-01-with-description` story proves the fixture in both renderers.
