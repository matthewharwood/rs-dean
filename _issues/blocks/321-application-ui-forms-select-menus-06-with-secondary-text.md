...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/forms/select-menus#component-71680e7ae17adab2886761e3e2390f6a

Catalog path: Application UI / forms / Select Menus / variant 6 of 7

## TPM Outcome

Ship `application-ui-forms-select-menus-06-with-secondary-text` as a registered Rust-first block in the Select Menus family. Preserve
...
- Block index `320` and `application-ui-forms-select-menus-06-with-secondary-text` are the stable registry identity.
...
- Resolve `BlockId::from_index(320)`, or look up `application-ui-forms-select-menus-06-with-secondary-text` with
...
- The registry resolves both block id `320` and slug `application-ui-forms-select-menus-06-with-secondary-text` one-to-one.
...
- An isolated `block-application-ui-forms-select-menus-06-with-secondary-text` story proves the fixture in both renderers.
