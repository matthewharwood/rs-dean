...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/forms/select-menus#component-e0f6280e5501d69e188768909fda485e

Catalog path: Application UI / forms / Select Menus / variant 4 of 7

## TPM Outcome

Ship `application-ui-forms-select-menus-04-custom-with-status-indicator` as a registered Rust-first block in the Select Menus family. Preserve
...
- Block index `318` and `application-ui-forms-select-menus-04-custom-with-status-indicator` are the stable registry identity.
...
- Resolve `BlockId::from_index(318)`, or look up `application-ui-forms-select-menus-04-custom-with-status-indicator` with
...
- The registry resolves both block id `318` and slug `application-ui-forms-select-menus-04-custom-with-status-indicator` one-to-one.
...
- An isolated `block-application-ui-forms-select-menus-04-custom-with-status-indicator` story proves the fixture in both renderers.
