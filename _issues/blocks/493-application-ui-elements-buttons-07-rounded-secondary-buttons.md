...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/elements/buttons#component-652008cd95432d6261e7cff19e19bc63

Catalog path: Application UI / elements / Buttons / variant 7 of 8

## TPM Outcome

Ship `application-ui-elements-buttons-07-rounded-secondary-buttons` as a registered Rust-first block in the Buttons family. Preserve
...
- Block index `492` and `application-ui-elements-buttons-07-rounded-secondary-buttons` are the stable registry identity.
...
- Resolve `BlockId::from_index(492)`, or look up `application-ui-elements-buttons-07-rounded-secondary-buttons` with
...
- The registry resolves both block id `492` and slug `application-ui-elements-buttons-07-rounded-secondary-buttons` one-to-one.
...
- An isolated `block-application-ui-elements-buttons-07-rounded-secondary-buttons` story proves the fixture in both renderers.
