...
Source: https://tailwindcss.com/plus/ui-blocks/ecommerce/components/order-history#component-4a9dcf75e8e751856f47d68ae0b51eba

Catalog path: Ecommerce / components / Order History / variant 3 of 4

## TPM Outcome

Ship `ecommerce-components-order-history-03-invoice-list` as a registered Rust-first block in the Order History family. Preserve
...
- Block index `617` and `ecommerce-components-order-history-03-invoice-list` are the stable registry identity.
...
- Resolve `BlockId::from_index(617)`, or look up `ecommerce-components-order-history-03-invoice-list` with
...
- The registry resolves both block id `617` and slug `ecommerce-components-order-history-03-invoice-list` one-to-one.
...
- An isolated `block-ecommerce-components-order-history-03-invoice-list` story proves the fixture in both renderers.
