...
Source: https://tailwindcss.com/plus/ui-blocks/marketing/elements/headers#component-ee5df49936fc6550a95895277c81f43c

Catalog path: Marketing / elements / Headers / variant 6 of 11

## TPM Outcome

Ship `marketing-elements-headers-06-with-call-to-action` as a registered Rust-first block in the Headers family. Preserve
...
- Block index `138` and `marketing-elements-headers-06-with-call-to-action` are the stable registry identity.
...
- Resolve `BlockId::from_index(138)`, or look up `marketing-elements-headers-06-with-call-to-action` with
...
- The registry resolves both block id `138` and slug `marketing-elements-headers-06-with-call-to-action` one-to-one.
...
- An isolated `block-marketing-elements-headers-06-with-call-to-action` story proves the fixture in both renderers.
