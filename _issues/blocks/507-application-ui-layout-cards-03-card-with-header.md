...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/layout/cards#component-4beba7dd94a1e5d6fd88a6d46236b4b7

Catalog path: Application UI / layout / Cards / variant 3 of 10

## TPM Outcome

Ship `application-ui-layout-cards-03-card-with-header` as a registered Rust-first block in the Cards family. Preserve
...
- Block index `506` and `application-ui-layout-cards-03-card-with-header` are the stable registry identity.
...
- Resolve `BlockId::from_index(506)`, or look up `application-ui-layout-cards-03-card-with-header` with
...
- The registry resolves both block id `506` and slug `application-ui-layout-cards-03-card-with-header` one-to-one.
...
- An isolated `block-application-ui-layout-cards-03-card-with-header` story proves the fixture in both renderers.
