...
Source: https://tailwindcss.com/plus/ui-blocks/ecommerce/page-examples/checkout-pages#component-f5dffb97e3fb18b74edb4ca4864cb359

Catalog path: Ecommerce / page-examples / Checkout Pages / variant 1 of 5

## TPM Outcome

Ship `ecommerce-page-examples-checkout-pages-01-with-order-summary-sidebar` as a registered Rust-first block in the Checkout Pages family. Preserve
the source block's Sidebar end composition intent while using original fixture
...
- Use the shared Section -> Container -> Grid/GridItem -> Stack/Cluster box
  model with the `Sidebar end` preset.
...
- Block index `644` and `ecommerce-page-examples-checkout-pages-01-with-order-summary-sidebar` are the stable registry identity.
...
- Resolve `BlockId::from_index(644)`, or look up `ecommerce-page-examples-checkout-pages-01-with-order-summary-sidebar` with
...
- The registry resolves both block id `644` and slug `ecommerce-page-examples-checkout-pages-01-with-order-summary-sidebar` one-to-one.
...
- An isolated `block-ecommerce-page-examples-checkout-pages-01-with-order-summary-sidebar` story proves the fixture in both renderers.
