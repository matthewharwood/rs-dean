...
Source: https://tailwindcss.com/plus/ui-blocks/ecommerce/components/category-filters#component-8dbbe78f722647bcb452fab9ba8a3b9a

Catalog path: Ecommerce / components / Category Filters / variant 3 of 5

## TPM Outcome

Ship `ecommerce-components-category-filters-03-with-expandable-product-filter-panel` as a registered Rust-first block in the Category Filters family. Preserve
the source block's Split composition intent while using original fixture
...
- Use the shared Section -> Container -> Grid/GridItem -> Stack/Cluster box
  model with the `Split` preset.
- Apply the typed `Product media` media mode, `Panel` surface treatment, and
...
- Block index `573` and `ecommerce-components-category-filters-03-with-expandable-product-filter-panel` are the stable registry identity.
...
- Variant behavior is constrained by `Product media`, `Panel`, and `Filter`
...
- Resolve `BlockId::from_index(573)`, or look up `ecommerce-components-category-filters-03-with-expandable-product-filter-panel` with
...
- The registry resolves both block id `573` and slug `ecommerce-components-category-filters-03-with-expandable-product-filter-panel` one-to-one.
...
- An isolated `block-ecommerce-components-category-filters-03-with-expandable-product-filter-panel` story proves the fixture in both renderers.
