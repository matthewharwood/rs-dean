...
Source: https://tailwindcss.com/plus/ui-blocks/marketing/sections/logo-clouds#component-58e002f268559c7aba120b917e51ea65

Catalog path: Marketing / sections / Logo Clouds / variant 4 of 6

## TPM Outcome

Ship `marketing-sections-logo-clouds-04-split-with-logos-on-right` as a registered Rust-first block in the Logo Clouds family. Preserve
...
- Block index `116` and `marketing-sections-logo-clouds-04-split-with-logos-on-right` are the stable registry identity.
...
- Variant behavior is constrained by `Logo`, `Plain`, and `Static`
...
- Resolve `BlockId::from_index(116)`, or look up `marketing-sections-logo-clouds-04-split-with-logos-on-right` with
...
- The registry resolves both block id `116` and slug `marketing-sections-logo-clouds-04-split-with-logos-on-right` one-to-one.
...
- An isolated `block-marketing-sections-logo-clouds-04-split-with-logos-on-right` story proves the fixture in both renderers.
