...
Source: https://tailwindcss.com/plus/ui-blocks/marketing/sections/newsletter-sections#component-0199bc4ef95ddc2e9442384f5d91f6a6

Catalog path: Marketing / sections / Newsletter Sections / variant 3 of 6

## TPM Outcome

Ship `marketing-sections-newsletter-sections-03-simple-side-by-side-on-brand` as a registered Rust-first block in the Newsletter Sections family. Preserve
the source block's Twelve column composition intent while using original fixture
...
- Use the shared Section -> Container -> Grid/GridItem -> Stack/Cluster box
  model with the `Twelve column` preset.
- Apply the typed `No media` media mode, `Brand` surface treatment, and
...
- Block index `63` and `marketing-sections-newsletter-sections-03-simple-side-by-side-on-brand` are the stable registry identity.
...
- The family maps to the `Form` composition pattern and the `Input`
  primary UI component.
- Variant behavior is constrained by `No media`, `Brand`, and `Form`
...
- Resolve `BlockId::from_index(63)`, or look up `marketing-sections-newsletter-sections-03-simple-side-by-side-on-brand` with
...
- The registry resolves both block id `63` and slug `marketing-sections-newsletter-sections-03-simple-side-by-side-on-brand` one-to-one.
...
- An isolated `block-marketing-sections-newsletter-sections-03-simple-side-by-side-on-brand` story proves the fixture in both renderers.
