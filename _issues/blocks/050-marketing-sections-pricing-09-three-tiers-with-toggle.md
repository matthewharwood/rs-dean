...
Source: https://tailwindcss.com/plus/ui-blocks/marketing/sections/pricing#component-35c0d8b89ac8b51d8ff6ec404f3c5b99

Catalog path: Marketing / sections / Pricing Sections / variant 9 of 12

## TPM Outcome

Ship `marketing-sections-pricing-09-three-tiers-with-toggle` as a registered Rust-first block in the Pricing Sections family. Preserve
...
- Apply the typed `No media` media mode, `Plain` surface treatment, and
  `Toggle` interaction contract without adding one-off classes.
...
- Block index `49` and `marketing-sections-pricing-09-three-tiers-with-toggle` are the stable registry identity.
...
- Variant behavior is constrained by `No media`, `Plain`, and `Toggle`
...
- Resolve `BlockId::from_index(49)`, or look up `marketing-sections-pricing-09-three-tiers-with-toggle` with
...
- The registry resolves both block id `49` and slug `marketing-sections-pricing-09-three-tiers-with-toggle` one-to-one.
...
- An isolated `block-marketing-sections-pricing-09-three-tiers-with-toggle` story proves the fixture in both renderers.
