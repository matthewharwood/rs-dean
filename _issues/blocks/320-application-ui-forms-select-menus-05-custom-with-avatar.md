...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/forms/select-menus#component-71d9116be789a254c260369f03472985

Catalog path: Application UI / forms / Select Menus / variant 5 of 7

## TPM Outcome

Ship `application-ui-forms-select-menus-05-custom-with-avatar` as a registered Rust-first block in the Select Menus family. Preserve
...
- Block index `319` and `application-ui-forms-select-menus-05-custom-with-avatar` are the stable registry identity.
...
- The family maps to the `Form` composition pattern and the `Select`
...
- Resolve `BlockId::from_index(319)`, or look up `application-ui-forms-select-menus-05-custom-with-avatar` with
...
- The registry resolves both block id `319` and slug `application-ui-forms-select-menus-05-custom-with-avatar` one-to-one.
...
- An isolated `block-application-ui-forms-select-menus-05-custom-with-avatar` story proves the fixture in both renderers.
