...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/feedback/empty-states#component-77a8b943310fbb088169cf88b4df9eb5

Catalog path: Application UI / feedback / Empty States / variant 6 of 6

## TPM Outcome

Ship `application-ui-feedback-empty-states-06-with-recommendations-grid` as a registered Rust-first block in the Empty States family. Preserve
...
- Block index `375` and `application-ui-feedback-empty-states-06-with-recommendations-grid` are the stable registry identity.
...
- The family maps to the `Empty state` composition pattern and the `Empty`
...
- Resolve `BlockId::from_index(375)`, or look up `application-ui-feedback-empty-states-06-with-recommendations-grid` with
...
- The registry resolves both block id `375` and slug `application-ui-feedback-empty-states-06-with-recommendations-grid` one-to-one.
...
- An isolated `block-application-ui-feedback-empty-states-06-with-recommendations-grid` story proves the fixture in both renderers.
