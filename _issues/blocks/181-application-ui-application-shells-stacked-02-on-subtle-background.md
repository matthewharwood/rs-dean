...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/application-shells/stacked#component-f87c42024bb0616c75a045c3be2e9157

Catalog path: Application UI / application-shells / Stacked Layouts / variant 2 of 9

## TPM Outcome

Ship `application-ui-application-shells-stacked-02-on-subtle-background` as a registered Rust-first block in the Stacked Layouts family. Preserve
...
- Apply the typed `No media` media mode, `Subtle` surface treatment, and
...
- Block index `180` and `application-ui-application-shells-stacked-02-on-subtle-background` are the stable registry identity.
...
- The family maps to the `Application shell` composition pattern and the `Navigation Menu`
  primary UI component.
- Variant behavior is constrained by `No media`, `Subtle`, and `Static`
...
- Resolve `BlockId::from_index(180)`, or look up `application-ui-application-shells-stacked-02-on-subtle-background` with
...
- The registry resolves both block id `180` and slug `application-ui-application-shells-stacked-02-on-subtle-background` one-to-one.
...
- An isolated `block-application-ui-application-shells-stacked-02-on-subtle-background` story proves the fixture in both renderers.
