...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/elements/button-groups#component-0d33369a2dac81cc4ed2f8b1e997a9cb

Catalog path: Application UI / elements / Button Groups / variant 2 of 5

## TPM Outcome

Ship `application-ui-elements-button-groups-02-icon-only` as a registered Rust-first block in the Button Groups family. Preserve
...
- Block index `495` and `application-ui-elements-button-groups-02-icon-only` are the stable registry identity.
...
- Resolve `BlockId::from_index(495)`, or look up `application-ui-elements-button-groups-02-icon-only` with
...
- The registry resolves both block id `495` and slug `application-ui-elements-button-groups-02-icon-only` one-to-one.
...
- An isolated `block-application-ui-elements-button-groups-02-icon-only` story proves the fixture in both renderers.
