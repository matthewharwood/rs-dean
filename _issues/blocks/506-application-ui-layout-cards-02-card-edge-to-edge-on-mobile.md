...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/layout/cards#component-5aa8234cc604a3b9398cf028559e4148

Catalog path: Application UI / layout / Cards / variant 2 of 10

## TPM Outcome

Ship `application-ui-layout-cards-02-card-edge-to-edge-on-mobile` as a registered Rust-first block in the Cards family. Preserve
...
- Block index `505` and `application-ui-layout-cards-02-card-edge-to-edge-on-mobile` are the stable registry identity.
...
- The family maps to the `Layout` composition pattern and the `Card`
...
- Resolve `BlockId::from_index(505)`, or look up `application-ui-layout-cards-02-card-edge-to-edge-on-mobile` with
...
- The registry resolves both block id `505` and slug `application-ui-layout-cards-02-card-edge-to-edge-on-mobile` one-to-one.
...
- An isolated `block-application-ui-layout-cards-02-card-edge-to-edge-on-mobile` story proves the fixture in both renderers.
