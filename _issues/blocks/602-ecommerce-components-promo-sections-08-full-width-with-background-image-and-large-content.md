...
Source: https://tailwindcss.com/plus/ui-blocks/ecommerce/components/promo-sections#component-2c7fddc98add76710894f66c325afcd7

Catalog path: Ecommerce / components / Promo Sections / variant 8 of 8

## TPM Outcome

Ship `ecommerce-components-promo-sections-08-full-width-with-background-image-and-large-content` as a registered Rust-first block in the Promo Sections family. Preserve
the source block's Full width composition intent while using original fixture
...
- Use the shared Section -> Container -> Grid/GridItem -> Stack/Cluster box
  model with the `Full width` preset.
- Apply the typed `Background image` media mode, `Plain` surface treatment, and
...
- Block index `601` and `ecommerce-components-promo-sections-08-full-width-with-background-image-and-large-content` are the stable registry identity.
...
- The family maps to the `Commerce feature` composition pattern and the `Card`
  primary UI component.
- Variant behavior is constrained by `Background image`, `Plain`, and `Actions`
...
- Resolve `BlockId::from_index(601)`, or look up `ecommerce-components-promo-sections-08-full-width-with-background-image-and-large-content` with
...
- The registry resolves both block id `601` and slug `ecommerce-components-promo-sections-08-full-width-with-background-image-and-large-content` one-to-one.
...
- An isolated `block-ecommerce-components-promo-sections-08-full-width-with-background-image-and-large-content` story proves the fixture in both renderers.
