...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/elements/badges#component-f108edb16e7f08bc619f3e3f9631cfbf

Catalog path: Application UI / elements / Badges / variant 5 of 16

## TPM Outcome

Ship `application-ui-elements-badges-05-with-border-and-remove-button` as a registered Rust-first block in the Badges family. Preserve
...
- Block index `469` and `application-ui-elements-badges-05-with-border-and-remove-button` are the stable registry identity.
...
- The family maps to the `Component composition` composition pattern and the `Badge`
...
- Resolve `BlockId::from_index(469)`, or look up `application-ui-elements-badges-05-with-border-and-remove-button` with
...
- The registry resolves both block id `469` and slug `application-ui-elements-badges-05-with-border-and-remove-button` one-to-one.
...
- An isolated `block-application-ui-elements-badges-05-with-border-and-remove-button` story proves the fixture in both renderers.
