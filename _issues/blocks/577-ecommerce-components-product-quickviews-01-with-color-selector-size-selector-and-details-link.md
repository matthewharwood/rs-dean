...
Source: https://tailwindcss.com/plus/ui-blocks/ecommerce/components/product-quickviews#component-4ae074374a0cc700a56c35902dc52a5e

Catalog path: Ecommerce / components / Product Quickviews / variant 1 of 4

## TPM Outcome

Ship `ecommerce-components-product-quickviews-01-with-color-selector-size-selector-and-details-link` as a registered Rust-first block in the Product Quickviews family. Preserve
the source block's Overlay composition intent while using original fixture
...
- Use the shared Section -> Container -> Grid/GridItem -> Stack/Cluster box
  model with the `Overlay` preset.
- Apply the typed `Product media` media mode, `Plain` surface treatment, and
  `Overlay` interaction contract without adding one-off classes.
...
- Block index `576` and `ecommerce-components-product-quickviews-01-with-color-selector-size-selector-and-details-link` are the stable registry identity.
...
- The family maps to the `Overlay` composition pattern and the `Dialog`
  primary UI component.
- Variant behavior is constrained by `Product media`, `Plain`, and `Overlay`
...
- Resolve `BlockId::from_index(576)`, or look up `ecommerce-components-product-quickviews-01-with-color-selector-size-selector-and-details-link` with
...
- The registry resolves both block id `576` and slug `ecommerce-components-product-quickviews-01-with-color-selector-size-selector-and-details-link` one-to-one.
...
- An isolated `block-ecommerce-components-product-quickviews-01-with-color-selector-size-selector-and-details-link` story proves the fixture in both renderers.
