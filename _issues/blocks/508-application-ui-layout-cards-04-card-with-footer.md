...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/layout/cards#component-0e734eb30dc9a8ca93b67f3fbfad084c

Catalog path: Application UI / layout / Cards / variant 4 of 10

## TPM Outcome

Ship `application-ui-layout-cards-04-card-with-footer` as a registered Rust-first block in the Cards family. Preserve
...
- Block index `507` and `application-ui-layout-cards-04-card-with-footer` are the stable registry identity.
...
- Resolve `BlockId::from_index(507)`, or look up `application-ui-layout-cards-04-card-with-footer` with
...
- The registry resolves both block id `507` and slug `application-ui-layout-cards-04-card-with-footer` one-to-one.
...
- An isolated `block-application-ui-layout-cards-04-card-with-footer` story proves the fixture in both renderers.
