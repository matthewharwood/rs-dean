...
Source: https://tailwindcss.com/plus/ui-blocks/ecommerce/components/order-summaries#component-f6f89cfa300f93a2a91b2000a955e617

Catalog path: Ecommerce / components / Order Summaries / variant 1 of 4

## TPM Outcome

Ship `ecommerce-components-order-summaries-01-with-split-image` as a registered Rust-first block in the Order Summaries family. Preserve
...
- Block index `611` and `ecommerce-components-order-summaries-01-with-split-image` are the stable registry identity.
...
- Resolve `BlockId::from_index(611)`, or look up `ecommerce-components-order-summaries-01-with-split-image` with
...
- The registry resolves both block id `611` and slug `ecommerce-components-order-summaries-01-with-split-image` one-to-one.
...
- An isolated `block-ecommerce-components-order-summaries-01-with-split-image` story proves the fixture in both renderers.
