...
Source: https://tailwindcss.com/plus/ui-blocks/ecommerce/components/category-filters#component-e799517f86cc2fc79bf0eebb45c16eea

Catalog path: Ecommerce / components / Category Filters / variant 4 of 5

## TPM Outcome

Ship `ecommerce-components-category-filters-04-with-inline-actions-and-expandable-sidebar-filters` as a registered Rust-first block in the Category Filters family. Preserve
the source block's Sidebar start composition intent while using original fixture
...
- Use the shared Section -> Container -> Grid/GridItem -> Stack/Cluster box
  model with the `Sidebar start` preset.
- Apply the typed `No media` media mode, `Plain` surface treatment, and
  `Filter` interaction contract without adding one-off classes.
...
- Block index `574` and `ecommerce-components-category-filters-04-with-inline-actions-and-expandable-sidebar-filters` are the stable registry identity.
...
- The family maps to the `Form` composition pattern and the `Checkbox`
  primary UI component.
- Variant behavior is constrained by `No media`, `Plain`, and `Filter`
...
- Resolve `BlockId::from_index(574)`, or look up `ecommerce-components-category-filters-04-with-inline-actions-and-expandable-sidebar-filters` with
...
- The registry resolves both block id `574` and slug `ecommerce-components-category-filters-04-with-inline-actions-and-expandable-sidebar-filters` one-to-one.
...
- An isolated `block-ecommerce-components-category-filters-04-with-inline-actions-and-expandable-sidebar-filters` story proves the fixture in both renderers.
