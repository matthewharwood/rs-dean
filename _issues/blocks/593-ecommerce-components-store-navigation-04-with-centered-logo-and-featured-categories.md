...
Source: https://tailwindcss.com/plus/ui-blocks/ecommerce/components/store-navigation#component-bbfaffb883e108073b3253d5c42f479f

Catalog path: Ecommerce / components / Store Navigation / variant 4 of 5

## TPM Outcome

Ship `ecommerce-components-store-navigation-04-with-centered-logo-and-featured-categories` as a registered Rust-first block in the Store Navigation family. Preserve
the source block's Centered composition intent while using original fixture
...
- Use the shared Section -> Container -> Grid/GridItem -> Stack/Cluster box
  model with the `Centered` preset.
- Apply the typed `Logo` media mode, `Plain` surface treatment, and
  `Menu` interaction contract without adding one-off classes.
...
- Block index `592` and `ecommerce-components-store-navigation-04-with-centered-logo-and-featured-categories` are the stable registry identity.
...
- The family maps to the `Navigation` composition pattern and the `Navigation Menu`
  primary UI component.
- Variant behavior is constrained by `Logo`, `Plain`, and `Menu`
...
- Resolve `BlockId::from_index(592)`, or look up `ecommerce-components-store-navigation-04-with-centered-logo-and-featured-categories` with
...
- The registry resolves both block id `592` and slug `ecommerce-components-store-navigation-04-with-centered-logo-and-featured-categories` one-to-one.
...
- An isolated `block-ecommerce-components-store-navigation-04-with-centered-logo-and-featured-categories` story proves the fixture in both renderers.
