...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/forms/input-groups#component-7cbbedb5c241c41093f698b30e1eff40

Catalog path: Application UI / forms / Input Groups / variant 4 of 21

## TPM Outcome

Ship `application-ui-forms-input-groups-04-input-with-disabled-state` as a registered Rust-first block in the Input Groups family. Preserve
...
- Block index `297` and `application-ui-forms-input-groups-04-input-with-disabled-state` are the stable registry identity.
...
- Resolve `BlockId::from_index(297)`, or look up `application-ui-forms-input-groups-04-input-with-disabled-state` with
...
- The registry resolves both block id `297` and slug `application-ui-forms-input-groups-04-input-with-disabled-state` one-to-one.
...
- An isolated `block-application-ui-forms-input-groups-04-input-with-disabled-state` story proves the fixture in both renderers.
