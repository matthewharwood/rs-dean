...
Source: https://tailwindcss.com/plus/ui-blocks/marketing/sections/header#component-e89a0157d7858d7ee8df1e9415a7c550

Catalog path: Marketing / sections / Header Sections / variant 8 of 8

## TPM Outcome

Ship `marketing-sections-header-08-centered-with-background-image` as a registered Rust-first block in the Header Sections family. Preserve
...
- Block index `60` and `marketing-sections-header-08-centered-with-background-image` are the stable registry identity.
...
- The family maps to the `Navigation` composition pattern and the `Item`
  primary UI component.
- Variant behavior is constrained by `Background image`, `Plain`, and `Menu`
...
- Resolve `BlockId::from_index(60)`, or look up `marketing-sections-header-08-centered-with-background-image` with
...
- The registry resolves both block id `60` and slug `marketing-sections-header-08-centered-with-background-image` one-to-one.
...
- An isolated `block-marketing-sections-header-08-centered-with-background-image` story proves the fixture in both renderers.
