...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/navigation/breadcrumbs#component-09fbe246cb968a4ab03a97c5da4947ee

Catalog path: Application UI / navigation / Breadcrumbs / variant 2 of 4

## TPM Outcome

Ship `application-ui-navigation-breadcrumbs-02-full-width-bar` as a registered Rust-first block in the Breadcrumbs family. Preserve
...
- Block index `411` and `application-ui-navigation-breadcrumbs-02-full-width-bar` are the stable registry identity.
...
- The family maps to the `Navigation` composition pattern and the `Breadcrumb`
...
- Resolve `BlockId::from_index(411)`, or look up `application-ui-navigation-breadcrumbs-02-full-width-bar` with
...
- The registry resolves both block id `411` and slug `application-ui-navigation-breadcrumbs-02-full-width-bar` one-to-one.
...
- An isolated `block-application-ui-navigation-breadcrumbs-02-full-width-bar` story proves the fixture in both renderers.
