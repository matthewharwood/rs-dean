...
Source: https://tailwindcss.com/plus/ui-blocks/marketing/sections/heroes#component-dc7ca59e587aa00e4aa8d78f6ed8aff3

Catalog path: Marketing / sections / Hero Sections / variant 4 of 12

## TPM Outcome

Ship `marketing-sections-heroes-04-split-with-code-example` as a registered Rust-first block in the Hero Sections family. Preserve
...
- Apply the typed `Code` media mode, `Plain` surface treatment, and
...
- Block index `3` and `marketing-sections-heroes-04-split-with-code-example` are the stable registry identity.
...
- Variant behavior is constrained by `Code`, `Plain`, and `Actions`
...
- Resolve `BlockId::from_index(3)`, or look up `marketing-sections-heroes-04-split-with-code-example` with
...
- The registry resolves both block id `3` and slug `marketing-sections-heroes-04-split-with-code-example` one-to-one.
...
- An isolated `block-marketing-sections-heroes-04-split-with-code-example` story proves the fixture in both renderers.
