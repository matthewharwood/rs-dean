...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/overlays/notifications#component-755581b198213fbd8058c85f7f18cb39

Catalog path: Application UI / overlays / Notifications / variant 3 of 6

## TPM Outcome

Ship `application-ui-overlays-notifications-03-with-actions-below` as a registered Rust-first block in the Notifications family. Preserve
...
- Block index `450` and `application-ui-overlays-notifications-03-with-actions-below` are the stable registry identity.
...
- The family maps to the `Feedback` composition pattern and the `Toast`
...
- Resolve `BlockId::from_index(450)`, or look up `application-ui-overlays-notifications-03-with-actions-below` with
...
- The registry resolves both block id `450` and slug `application-ui-overlays-notifications-03-with-actions-below` one-to-one.
...
- An isolated `block-application-ui-overlays-notifications-03-with-actions-below` story proves the fixture in both renderers.
