...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/forms/toggles#component-bcaf782196186836b6ea686e7096e734

Catalog path: Application UI / forms / Toggles / variant 3 of 5

## TPM Outcome

Ship `application-ui-forms-toggles-03-toggle-with-icon` as a registered Rust-first block in the Toggles family. Preserve
...
- Block index `349` and `application-ui-forms-toggles-03-toggle-with-icon` are the stable registry identity.
...
- The family maps to the `Form` composition pattern and the `Switch`
  primary UI component.
- Variant behavior is constrained by `Icon`, `Plain`, and `Toggle`
...
- Resolve `BlockId::from_index(349)`, or look up `application-ui-forms-toggles-03-toggle-with-icon` with
...
- The registry resolves both block id `349` and slug `application-ui-forms-toggles-03-toggle-with-icon` one-to-one.
...
- An isolated `block-application-ui-forms-toggles-03-toggle-with-icon` story proves the fixture in both renderers.
