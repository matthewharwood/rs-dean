...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/forms/select-menus#component-c549ac2695455cb78d529c3a00293fe0

Catalog path: Application UI / forms / Select Menus / variant 3 of 7

## TPM Outcome

Ship `application-ui-forms-select-menus-03-custom-with-check-on-left` as a registered Rust-first block in the Select Menus family. Preserve
...
- Block index `317` and `application-ui-forms-select-menus-03-custom-with-check-on-left` are the stable registry identity.
...
- Resolve `BlockId::from_index(317)`, or look up `application-ui-forms-select-menus-03-custom-with-check-on-left` with
...
- The registry resolves both block id `317` and slug `application-ui-forms-select-menus-03-custom-with-check-on-left` one-to-one.
...
- An isolated `block-application-ui-forms-select-menus-03-custom-with-check-on-left` story proves the fixture in both renderers.
