...
Source: https://tailwindcss.com/plus/ui-blocks/application-ui/layout/list-containers#component-22e9be21c06d9870c40b338e4e43a2a1

Catalog path: Application UI / layout / List containers / variant 5 of 7

## TPM Outcome

Ship `application-ui-layout-list-containers-05-separate-cards-full-width-on-mobile` as a registered Rust-first block in the List containers family. Preserve
...
- Apply the typed `No media` media mode, `Panel` surface treatment, and
...
- Block index `518` and `application-ui-layout-list-containers-05-separate-cards-full-width-on-mobile` are the stable registry identity.
...
- Variant behavior is constrained by `No media`, `Panel`, and `Static`
...
- Resolve `BlockId::from_index(518)`, or look up `application-ui-layout-list-containers-05-separate-cards-full-width-on-mobile` with
...
- The registry resolves both block id `518` and slug `application-ui-layout-list-containers-05-separate-cards-full-width-on-mobile` one-to-one.
...
- An isolated `block-application-ui-layout-list-containers-05-separate-cards-full-width-on-mobile` story proves the fixture in both renderers.
