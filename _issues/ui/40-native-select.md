...
Ship a Rust-first `Native Select` component that follows the shadcn composition model while using `rs-dean-ui` design tokens, shared component specs, and local-first state boundaries. A platform-native select control styled to match tokens.

## Design Considerations

- Use only the shared Tailwind token vocabulary for color, type, space, radius, shadow, and motion.
- Preserve shadcn's compositional anatomy: NativeSelect, NativeSelectTrigger, NativeSelectOption, NativeSelectValue.
...
- `Native Select` appears in the story gallery and `_issues` index.
