...
Ship a Rust-first `Button` component that follows the shadcn composition model while using `rs-dean-ui` design tokens, shared component specs, and local-first state boundaries. The primary action primitive for commands, links, and submit actions.

## Design Considerations

- Use only the shared Tailwind token vocabulary for color, type, space, radius, shadow, and motion.
- Preserve shadcn's compositional anatomy: Button, ButtonIcon, ButtonLabel.
...
- `Button` appears in the story gallery and `_issues` index.
...
- [x] Concrete typed model, part enum, render nodes, validation, local press state, link/submit/reset kind semantics, and named token-only Leptos component implemented.
- [x] Dedicated story coverage includes default, size/variant, link, loading, disabled, invalid, and theme-scoped examples.
