...
const TOGGLE_INDICATOR_DISABLED: &str = "inline-grid min-h-s min-w-s place-items-center rounded-pill bg-surface-3 px-2xs text-00 font-7 uppercase tracking-label text-text-disabled";
const TOGGLE_LABEL: &str = "truncate";
const TOGGLE_LABEL_DISABLED: &str = "truncate text-text-disabled";
const TOGGLE_META: &str = "flex min-w-0 flex-wrap items-center gap-2xs";
const TOGGLE_STATUS: &str = "rounded-pill border border-border-subtle bg-surface-2 px-2xs py-3xs text-00 font-7 uppercase tracking-label text-text-muted";
const TOGGLE_STATUS_PRESSED: &str = "rounded-pill border border-brand bg-primary-soft px-2xs py-3xs text-00 font-7 uppercase tracking-label text-text-1";
const TOGGLE_STATUS_INVALID: &str = "rounded-pill border border-danger bg-error-soft px-2xs py-3xs text-00 font-7 uppercase tracking-label text-text-1";
const TOGGLE_STATUS_DISABLED: &str = "rounded-pill border border-border-muted bg-surface-2 px-2xs py-3xs text-00 font-7 uppercase tracking-label text-text-disabled";
const TOGGLE_DETAIL: &str = "m-0 text-0 leading-0 text-text-2";
const TOGGLE_DETAIL_INVALID: &str = "m-0 text-0 leading-0 text-danger";
...
const TOGGLE_GROUP_TITLE_DISABLED: &str = "m-0 text-0 font-7 leading-0 text-text-disabled";
const TOGGLE_GROUP_STATUS: &str = "rounded-pill border border-border-subtle bg-surface-2 px-2xs py-3xs text-00 font-7 uppercase tracking-label text-text-muted";
const TOGGLE_GROUP_STATUS_SELECTED: &str = "rounded-pill border border-brand bg-primary-soft px-2xs py-3xs text-00 font-7 uppercase tracking-label text-text-1";
const TOGGLE_GROUP_STATUS_INVALID: &str = "rounded-pill border border-danger bg-error-soft px-2xs py-3xs text-00 font-7 uppercase tracking-label text-text-1";
const TOGGLE_GROUP_STATUS_DISABLED: &str = "rounded-pill border border-border-muted bg-surface-2 px-2xs py-3xs text-00 font-7 uppercase tracking-label text-text-disabled";
const TOGGLE_GROUP_LIST_HORIZONTAL: &str = "flex flex-wrap gap-2xs";
const TOGGLE_GROUP_LIST_VERTICAL: &str = "grid gap-2xs";
const TOGGLE_GROUP_DETAIL: &str = "m-0 text-0 leading-0 text-text-2";
...
#[component]
pub fn Toggle(#[prop(optional, default = default_toggle_model())] model: ToggleModel) -> AnyView {
    if let Err(report) = validate_toggle_model(&model) {
        let message = format!("Toggle validation failed: {report}");
        return view! {
            <div class=TOGGLE_ERROR data-ui-component="toggle" data-ui-state="invalid" role="alert">
                {message}
            </div>
        }
        .into_any();
    ...
    let root = nodes
        .iter()
        .find(|node| node.part == TogglePart::Root)
        .expect("invariant: toggle render nodes include root")
        .clone();
    let indicator = nodes
        .iter()
        .find(|node| node.part == TogglePart::Indicator)
        .expect("invariant: toggle render nodes include indicator")
        .clone();
    let label = nodes
        .iter()
        .find(|node| node.part == TogglePart::Label)
        .expect("invariant: toggle render nodes include label")
        .clone();
    let root_value = root.value.clone();
    let indicator_value = indicator.value.clone();
    let label_value = label.value.clone();
    let root_label = root.label.clone();
    let label_copy = label.label.clone();
    let detail_copy = model.detail.clone();
    let error_detail = model.error.clone().unwrap_or_default();
    let pressed_status_for_data = model.pressed_label.clone();
    let unpressed_status_for_data = model.unpressed_label.clone();
    let pressed_status_for_text = model.pressed_label.clone();
    let unpressed_status_for_text = model.unpressed_label.clone();
    let pressed_indicator_for_text = model.pressed_indicator.clone();
    let unpressed_indicator_for_text = model.unpressed_indicator.clone();
    let (state, set_state) = signal(model.state());
    ...
    view! {
            ...
            data-ui-state=move || {
                state.with(|state| {
                    toggle_state_label(
                        ...
                        invalid,
                        state.pressed(),
                        state.is_active(TogglePart::Root),
                    )
                    .to_owned()
                })
            }
            data-ui-pressed=move || state.with(|state| state.pressed().label().to_owned())
            data-ui-value=root_value
            data-ui-status=move || {
                state.with(|state| {
                    toggle_status_copy(
                        state.pressed(),
                        pressed_status_for_data.as_str(),
                        unpressed_status_for_data.as_str(),
                    )
                })
            }
            aria-disabled=blocked.to_string()
        ...
        >
            <button
                ...
                data-ui-part=TogglePart::Root.label()
                aria-label=root_label
                aria-pressed=move || state.with(|state| state.pressed().aria_pressed().to_owned())
                disabled=blocked
                on:focus=move |_| {
                    if !blocked {
                        set_state.update(|state| {
                            let _ = state.apply(ToggleIntent::Focus(TogglePart::Root));
                        });
                    }
                }
                on:blur=move |_| {
                    if !blocked {
                        set_state.update(|state| {
                            let _ = state.apply(ToggleIntent::Blur);
                        });
                    }
                }
                on:click=move |_| {
                    if !blocked {
                        set_state.update(|state| {
                            let _ = state.apply(ToggleIntent::Toggle);
                        });
                    }
                }
            >
                <span
                    class=move || {
                        state.with(|state| {
                            toggle_indicator_class(state.pressed(), blocked, invalid).to_owned()
                        })
                    }
                    data-ui-part=TogglePart::Indicator.label()
                    data-ui-value=indicator_value
                    aria-hidden="true"
                >
                    {move || {
                        state.with(|state| {
                            toggle_indicator_copy(
                                state.pressed(),
                                pressed_indicator_for_text.as_str(),
                                unpressed_indicator_for_text.as_str(),
                            )
                        })
                    }}
                </span>
                <span
                    class=toggle_label_class(blocked)
                    data-ui-part=TogglePart::Label.label()
                    data-ui-value=label_value
                >
                    {label_copy}
                </span>
            </button>
            <div class=TOGGLE_META>
                <span
                    class=move || {
                        state.with(|state| {
                            toggle_status_class(state.pressed(), blocked, invalid).to_owned()
                        })
                    }
                >
                    {move || {
                        state.with(|state| {
                            toggle_status_copy(
                                state.pressed(),
                                pressed_status_for_text.as_str(),
                                unpressed_status_for_text.as_str(),
                            )
                        })
                    }}
                </span>
                <p class=toggle_detail_class(blocked, invalid)>{detail_copy}</p>
            </div>
            {invalid.then_some(view! { <p class=TOGGLE_ERROR>{error_detail}</p> })}
        </div>
    }
    .into_any()
}

const fn toggle_button_class(
    ...
    if disabled {
        return match density {
            ToggleDensity::Standard => TOGGLE_BUTTON_DISABLED,
            ToggleDensity::Dense => TOGGLE_BUTTON_DISABLED_DENSE,
        };
    }
    if invalid {
        return match density {
            ToggleDensity::Standard => TOGGLE_BUTTON_INVALID,
            ToggleDensity::Dense => TOGGLE_BUTTON_INVALID_DENSE,
        };
    }
    if active {
        return match density {
            ToggleDensity::Standard => TOGGLE_BUTTON_ACTIVE,
            ToggleDensity::Dense => TOGGLE_BUTTON_ACTIVE_DENSE,
        };
    }
    if pressed.is_pressed() {
        return match density {
            ToggleDensity::Standard => TOGGLE_BUTTON_PRESSED,
            ToggleDensity::Dense => TOGGLE_BUTTON_PRESSED_DENSE,
        };
    }
    match (density, variant) {
        (ToggleDensity::Standard, ToggleVariant::Default) => TOGGLE_BUTTON,
        (ToggleDensity::Dense, ToggleVariant::Default) => TOGGLE_BUTTON_DENSE,
        (ToggleDensity::Standard, ToggleVariant::Outline) => TOGGLE_BUTTON_OUTLINE,
        (ToggleDensity::Dense, ToggleVariant::Outline) => TOGGLE_BUTTON_OUTLINE_DENSE,
    }
}

const fn toggle_indicator_class(
    pressed: TogglePressed,
    disabled: bool,
    invalid: bool,
) -> &'static str {
    if disabled {
        return TOGGLE_INDICATOR_DISABLED;
    }
    if invalid {
        return TOGGLE_INDICATOR_INVALID;
    }
    if pressed.is_pressed() {
        TOGGLE_INDICATOR_PRESSED
    } else {
        TOGGLE_INDICATOR
    }
}

const fn toggle_label_class(disabled: bool) -> &'static str {
    if disabled {
...
const fn toggle_status_class(
    pressed: TogglePressed,
    disabled: bool,
    invalid: bool,
) -> &'static str {
    if disabled {
        TOGGLE_STATUS_DISABLED
    } else if invalid {
        TOGGLE_STATUS_INVALID
    } else if pressed.is_pressed() {
        TOGGLE_STATUS_PRESSED
    } else {
        TOGGLE_STATUS
    }
}

const fn toggle_detail_class(disabled: bool, invalid: bool) -> &'static str {
    if disabled {
        TOGGLE_DETAIL_DISABLED
    } else if invalid {
        TOGGLE_DETAIL_INVALID
    } else {
        TOGGLE_DETAIL
    }
}

fn toggle_status_copy(
    pressed: TogglePressed,
    pressed_label: &str,
    unpressed_label: &str,
) -> String {
    match pressed {
        TogglePressed::Unpressed => unpressed_label.to_owned(),
        TogglePressed::Pressed => pressed_label.to_owned(),
    }
}

fn toggle_indicator_copy(
    pressed: TogglePressed,
    pressed_indicator: &str,
    unpressed_indicator: &str,
) -> String {
    match pressed {
        TogglePressed::Unpressed => unpressed_indicator.to_owned(),
        TogglePressed::Pressed => pressed_indicator.to_owned(),
    }
}

const fn toggle_state_label(
    loading: bool,
    disabled: bool,
    invalid: bool,
    pressed: TogglePressed,
...
#[component]
pub fn ToggleGroup(
    #[prop(optional, default = default_toggle_group_model())] model: ToggleGroupModel,
) -> AnyView {
    if let Err(report) = validate_toggle_group_model(&model) {
        let message = format!("ToggleGroup validation failed: {report}");
        return view! {
            <div class=TOGGLE_GROUP_ERROR data-ui-component="toggle-group" data-ui-state="invalid" role="alert">
                {message}
            </div>
        }
        .into_any();
    ...
    let density = model.density;
    let variant = model.variant;
    let selection_mode = model.selection_mode;
    let orientation = model.orientation;
    let loading = model.loading;
    let disabled = model.disabled;
    let invalid = model.error.is_some();
    let blocked = loading || disabled;
    let state_model = model.state();
    let nodes = toggle_group_render_nodes(&model, &state_model);
    let root = nodes
        .iter()
    ...
    view! {
        <section
            class=toggle_group_root_class(density, invalid, disabled)
            data-ui-component="toggle-group"
            data-ui-part=ToggleGroupPart::Root.label()
            data-ui-density=density.label()
            data-ui-variant=variant.label()
            data-ui-selection-mode=selection_mode.label()
            data-ui-orientation=orientation.label()
            data-ui-state=move || {
                state.with(|state| {
                    toggle_group_state_label(
                        loading,
                        disabled,
                        invalid,
            ...
            data-ui-value=root_value
            data-ui-selected=move || {
                state.with(|state| toggle_group_selected_values_label(state.selected_values()))
            }
            role="group"
            aria-label=root_aria_label
            aria-disabled=blocked.to_string()
            ...
            aria-invalid=invalid.to_string()
        >
            <div class=TOGGLE_GROUP_HEADER>
                <p class=toggle_group_title_class(density, disabled)>{root_label}</p>
                <span
                    class=move || {
                        state.with(|state| {
                            toggle_group_status_class(
                                !state.selected_values().is_empty(),
                                invalid,
                                blocked,
                            )
            ...
            <div class=toggle_group_list_class(orientation)>
                {items
                    .into_iter()
                    .enumerate()
                    .map(|(index, item)| {
                        let value_for_data = item.value.clone();
                        let value_for_class = item.value.clone();
                        let value_for_pressed = item.value.clone();
                        let value_for_focus = item.value.clone();
                        let value_for_click = item.value.clone();
                        let value_for_indicator_class = item.value.clone();
                        let value_for_indicator_data = item.value.clone();
                        let value_for_indicator_copy = item.value.clone();
                        let item_disabled = blocked || item.disabled;
                        let label_for_aria = item.label.clone();
                        let label_for_text = item.label.clone();
                        let detail_for_title = item.detail.clone();
                        let detail_for_sr = item.detail.clone();
                        view! {
                            <button
                                type="button"
                                class=move || {
                                    state.with(|state| {
                                        let pressed = toggle_group_pressed_for_value(state, &value_for_class);
                                        toggle_button_class(
                                            density,
                                            variant,
                                            pressed,
                                            state.is_focused(&value_for_class),
                                            item_disabled,
                                            invalid,
                                        )
                                        .to_owned()
                                ...
                                data-ui-part=ToggleGroupPart::Item.label()
                                data-ui-value=value_for_data
                                data-ui-index=index.to_string()
                                aria-label=label_for_aria
                                aria-pressed=move || {
                                    state.with(|state| {
                                        toggle_group_pressed_for_value(state, &value_for_pressed)
                                            .aria_pressed()
                                            .to_owned()
                                    })
                                }
                                disabled=item_disabled
                                title=detail_for_title
                                on:focus=move |_| {
                                    if !item_disabled {
                                        let value = value_for_focus.clone();
                                        set_state.update(|state| {
                                ...
                                on:click=move |_| {
                                    if !item_disabled {
                                        ...
                                        set_state.update(|state| {
                                            let _ = state.apply(
                                                selection_mode,
                                                ToggleGroupIntent::Toggle(value),
                                            );
                                        });
                                    }
                                }
                            >
                                <span
                                    class=move || {
                                        state.with(|state| {
                                            toggle_indicator_class(
                                                toggle_group_pressed_for_value(
                                                    state,
                                                    &value_for_indicator_class,
                                                ),
                                                item_disabled,
                                                invalid,
                                            )
                                    ...
                                    data-ui-part=ToggleGroupPart::Indicator.label()
                                    data-ui-value=value_for_indicator_data
                                    aria-hidden="true"
                                >
                                    {move || {
                                        state.with(|state| {
                                ...
                                <span class=toggle_label_class(item_disabled)>{label_for_text}</span>
                                <span class="sr-only">{detail_for_sr}</span>
                            </button>
                        }
                    })
                    .collect_view()}
            </div>
            <p class=toggle_group_detail_class(invalid, blocked)>{root_detail}</p>
            {invalid.then_some(view! { <p class=TOGGLE_GROUP_ERROR>{error_detail}</p> })}
        </section>
    }
    .into_any()
}

const fn toggle_group_root_class(
    density: ToggleDensity,
    invalid: bool,
    disabled: bool,
) -> &'static str {
    if disabled {
        return TOGGLE_GROUP_ROOT_DISABLED;
    }
    if invalid {
        return TOGGLE_GROUP_ROOT_INVALID;
    }
    match density {
        ToggleDensity::Standard => TOGGLE_GROUP_ROOT,
        ToggleDensity::Dense => TOGGLE_GROUP_ROOT_DENSE,
    }
}

const fn toggle_group_title_class(density: ToggleDensity, disabled: bool) -> &'static str {
    if disabled {
        return TOGGLE_GROUP_TITLE_DISABLED;
    }
    match density {
        ToggleDensity::Standard => TOGGLE_GROUP_TITLE,
        ToggleDensity::Dense => TOGGLE_GROUP_TITLE_DENSE,
    }
}

const fn toggle_group_status_class(selected: bool, invalid: bool, disabled: bool) -> &'static str {
    if disabled {
        TOGGLE_GROUP_STATUS_DISABLED
    } else if invalid {
        TOGGLE_GROUP_STATUS_INVALID
    } else if selected {
        TOGGLE_GROUP_STATUS_SELECTED
    } else {
        TOGGLE_GROUP_STATUS
    }
}

const fn toggle_group_list_class(orientation: ToggleGroupOrientation) -> &'static str {
    match orientation {
        ToggleGroupOrientation::Horizontal => TOGGLE_GROUP_LIST_HORIZONTAL,
        ToggleGroupOrientation::Vertical => TOGGLE_GROUP_LIST_VERTICAL,
    }
}

const fn toggle_group_detail_class(invalid: bool, disabled: bool) -> &'static str {
    if disabled {
        TOGGLE_GROUP_DETAIL_DISABLED
    } else if invalid {
        TOGGLE_GROUP_DETAIL_INVALID
    } else {
        TOGGLE_GROUP_DETAIL
    }
}

fn toggle_group_pressed_for_value(state: &ToggleGroupState, value: &str) -> TogglePressed {
    if state.is_selected(value) {
